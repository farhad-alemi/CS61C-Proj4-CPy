...
#pragma omp parallel
        {
#pragma omp for
            for (int index = 0; index < (result->rows * result->cols) / (2 * STRIDE) * 2 * STRIDE; index += (2 * STRIDE)) {
                switch (operation) {
                    case '+':
                    ...
                    case '=':
                        ...
                        _mm256_storeu_pd(result->data + index + 12, _mm256_loadu_pd((const double *)(mat1->data + index + 12)));
                        break;
                    default:
                        break;
                }
    ...
    /* Tail Case */
    for (index = (result->rows * result->cols) / (2 * STRIDE) * 2 * STRIDE; index < result->rows * result->cols; ++index) {
        switch (operation) {
            case '+':
