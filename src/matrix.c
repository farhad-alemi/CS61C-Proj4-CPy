...
int pow_matrix(matrix *result, matrix *mat, int pow) {
    /* YOUR CODE HERE */
    int err_code, max_pow_needed, result_rows, result_cols, temp_matrix_rows, temp_matrix_cols;
...
#pragma omp parallel for
        ...
        if (pow - (1U << (size_t)max_pow_needed) > 0) {
            ...
            temp_matrix_rows = temp_matrix->rows;
            temp_matrix_cols = temp_matrix->cols;
            temp_matrix_data = temp_matrix->data;
        }

        for (int i = max_pow_needed - 1; i >= 0; --i) {
            if (binary_rep[i] == 1) {
                ...
                /* Register Blocking - This reduces the number of accesses to matrix fields. */
                mat1_cols = result_cols;
                mat1_data = result_data;
                mat2_cols = pow_2_matrices[i]->cols;
                mat2_data = pow_2_matrices[i]->data;
                res_rows = temp_matrix_rows;
                res_cols = temp_matrix_cols;
                res_data = temp_matrix_data;
                thresh_k = mat1_cols / 8 * 8;
                thresh_j = res_cols / 4 * 4;
